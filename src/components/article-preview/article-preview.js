      ...
      {position === 'dx' ? (
        <div className={`article ${position}`}>
          <div className="article__body">
            {article.subtitle && article.subtitle !== '' && <h5 className="article__body-subtitle">{article.subtitle}</h5>}
            ...
            {hasAction && customAction !== null ? (
              <a href={customAction} rel="noopener norefferer" className={`article__body-action ${position}`}>
                {actionText}
              </a>
            ) : hasAction ? (
              <Link className={`article__body-action ${position}`} to={`/blog/${article.slug}`}>
                {actionText}
              </Link>
            ) : null}
          </div>
